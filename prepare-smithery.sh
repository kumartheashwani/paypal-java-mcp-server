...
export SPRING_AUTOCONFIGURE_EXCLUDE=org.springframework.boot.autoconfigure.web.servlet.WebMvcAutoConfiguration
...
echo "   or with the command: java -Dspring.profiles.active=stdio -Dlogging.file.name=logs/mcp-server.log -Dlogging.level.com.example.mcpserver=DEBUG -Dspring.jmx.enabled=false -Dspring.autoconfigure.exclude=org.springframework.boot.autoconfigure.web.servlet.WebMvcAutoConfiguration -Dspring.mvc.async.request-timeout=60000 -jar app.jar" 